...
inline int __sub__(int a, const int b)
{
    a -= b;
    return a < 0 ? a + MOD : a;
}
...
inline void initialize()
{
    ...
    for (int i = 1, j; i < MAXN; ++i)
        for (j = 1; pentagon[j] <= i; ++j)
            partition_number[i] = dp_add[j] ? __add__(partition_number[i], partition_number[i - pentagon[j]])
...
int main()
{
    ...
    {
        scanf("%d", &num);
        int ans = (partition_number[num] + 1000000007) % 1000000007;
        printf("%d\n", ans);
    }
    return 0;
}